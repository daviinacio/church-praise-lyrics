...
export default function PraisesListPage() {
  ...
  async function handleMenuChangeStatus(praiseId, newStatus) {
    ...
    await api
      .put(`praises/${praiseId}`, {
        status: newStatus,
      })
      .then(() => {
        setPraises(
          praises.map((praise) => {
            if (praise.id === praiseId) praise.status = newStatus;
            return praise;
          })
        );
      })
  ...
  function handleMenuEdit(data) {
    if (editWithDialog) {
      ...
      handleMenuClose();
    } else router.push(`/praises/${data.id}/edit`);
  }
  ...
  function handleDialogEditorSave(data) {
    ...
    if (praises.some((e) => e.id === data.id)) {
      setPraises(
        praises.map((praise) => {
          if (praise.id === data.id) praise = data;
          return praise;
        })
      );
    } else {
  ...
  return (
    <>
      ...
      <AppBar>
        <Toolbar>
          <Typography variant="h6">Banco de louvor</Typography>
        </Toolbar>
        <Tabs
          value={tab}
        ...
        >
          {Object.keys(PraiseStatus).map((tabId, index) => (
            <TabPanel value={tab} index={index} showAll={false} key={tabId}>
                ...
                .map((item) => (
                  ...
                  >
                    <CardActionArea
                      ...
                      disabled={item.loadingLyricsPage === true}
                    >
                      <CardHeader
                        onClick={() => handleShowPraiseDetails(item.id)}
                        avatar={
                          <Avatar alt={item.tone} className={classes.tone}>
                            {item.tone}
                          </Avatar>
                        }
                        title={<span>{item.name}</span>}
                        subheader={<span>{item.artist}</span>}
                        // avatar={
              ...
              {praises.filter((item) => item.status === tabId).length === 0 &&
                !isLoading && (
                  <Typography>
                    {
                      [
      ...
      <Fab
        ...
        onClick={() => {
          if (editWithDialog) handleDialogEditorInflate(undefined);
          else router.push("/praises/new");
        }}
      >
        <AddIcon />
      </Fab>
      ...
      >
        {anchorData.status === PraiseStatus.SUGGESTION && (
          <MenuItem
            onClick={() =>
              handleMenuChangeStatus(anchorData.id, PraiseStatus.REHEARSING)
            }
          >
            Ensaiar
          </MenuItem>
        ...
        {anchorData.status === PraiseStatus.REHEARSING && (
          <MenuItem
            onClick={() => handleMenuChangeStatus(anchorData.id, "APPROVED")}
          >
            Aprovar
          </MenuItem>
        )}
        ...
        <MenuItem
          onClick={() => {
            handleMenuRemove(anchorData.id);
          }}
        >
          Remover
        </MenuItem>
